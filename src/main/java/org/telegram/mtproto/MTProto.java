...
public class MTProto {
    ...
    private final ConcurrentHashMap<Integer,TcpContext> contexts = new ConcurrentHashMap<Integer,TcpContext>();
    ...
    public void closeConnections() {
    	...
    	synchronized (this.contexts) {
            ...
            while(it.hasNext()){
            	...
            	TcpContext context = (TcpContext) pair.getValue();
    ...
    private class ConnectionFixerThread extends Thread {
        ...
        @Override
        public void run() {
        	//System.out.println("RUN proto.uuid= " + MTProto.this.uuid + " fixer uuid= " + this.uuid);
            setPriority(Thread.MIN_PRIORITY);
            while (!MTProto.this.isClosed) {
                if (Logger.LOG_THREADS) {
    ...
    private class TcpListener implements TcpContextCallback {
        ...
        public void onError(int errorCode, TcpContext context) {
            ...
            context.suspendConnection(true);
            //System.out.println("MTProto.onError contexts.connect");
            context.connect();
